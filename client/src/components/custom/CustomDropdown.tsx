...
const CustomDropdown = (props: ICustomDropdown) => {
  ...
  const selectOption = useCallback((e: Event) => {
    e.preventDefault();
    setStatus((prev: FireArmStatus) =>  prev === "active" ? "inactive" : "active")
  }, [setStatus]);
  
  return (
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        <Button variant="outline" className="font-inter capitalize w-40 justify-between px-3" id="status">
        {status} 
        <ChevronDown />
        </Button>
      </DropdownMenuTrigger>
      <DropdownMenuContent className="w-40 font-inter" align="start">
        <DropdownMenuGroup>
          ...
          {status === option && <Check className="ml-auto"/>} </DropdownMenuItem>))}
        </DropdownMenuGroup>
      </DropdownMenuContent>
    </DropdownMenu>
  )
